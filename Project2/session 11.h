...
//	char data[80];							/* 데이터 영역 : int형 데이터 저장 */
//	Node* next;							/* 포인터 영역 */
//};
...
//int addFirst(List* lp, char* data);			/* head node 뒤에 node 추가(역순 저장) */
//int  addLast(List* lp, char* data);			/* tail node 앞에 node 추가(정순 저장) */
//void displayList(List* lp);					/* 리스트 내의 모든 데이터 출력 */
//int removeNode(List* lp, char* data);		/* data 노드 삭제 */
